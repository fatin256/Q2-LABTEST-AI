...
# Put your JSON rules here as default
...
def evaluate_condition(facts: Dict[str, Any], cond: List[Any]) -> bool:
    """Evaluate one condition: [field, op, value]."""
    if len(cond) != 3:
        return False
    field, op, value = cond
...
def run_rules(facts: Dict[str, Any], rules: List[Dict[str, Any]]) -> Tuple[Dict[str, Any], List[Dict[str, Any]]]:
    """Return (best_action, fired_rules). Best action chosen by highest priority."""
    ...
    best_action = fired_sorted[0].get("action", {"ac_mode": "OFF", "fan_speed": "LOW", "setpoint": None, "reason": "No action"})
    return best_action, fired_sorted

# ----------------------------
# 2) Streamlit UI
# ----------------------------
st.set_page_config(page_title="Rule-Based AC Controller (Q2)", page_icon="❄️", layout="wide")
st.title("Q2: Rule-Based Smart Home Air Conditioner Controller")
st.caption("Uses your JSON rules (priority-based). Enter facts, click Evaluate, and view the matched rule(s).")

with st.sidebar:
    st.header("Environment Facts")

    temperature = st.number_input("Temperature (°C)", value=22.0)
    humidity = st.number_input("Humidity (%)", min_value=0.0, max_value=100.0, value=46.0)
    ...
    # True/False choice (as requested)
    windows_open = st.selectbox("Windows open?", options=[True, False], index=1)  # default False

    st.divider()
    st.header("Rules (JSON)")
    st.caption("Keep defaults or paste your JSON array of rules.")
...
try:
    ...
    if not isinstance(rules, list):
        raise ValueError("Rules must be a JSON array (list).")
...
if run:
    ...
    with col1:
        st.subheader("AC Decision (Best Rule)")
        ac_mode = action.get("ac_mode", "OFF")
        fan_speed = action.get("fan_speed", "LOW")
        setpoint = action.get("setpoint", None)
        reason = action.get("reason", "-")

        st.write(f"**AC Mode:** {ac_mode}")
        st.write(f"**Fan Speed:** {fan_speed}")
        st.write(f"**Setpoint:** {setpoint if setpoint is not None else '—'}")
